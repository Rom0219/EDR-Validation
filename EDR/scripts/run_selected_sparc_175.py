...
RESULTS_CSV = OUT_DIR.parent.parent / "data" / "sparc" / "sparc_results_full.csv" # Nuevo nombre de archivo para evitar sobrescribir
# Flag para saltar la generación de 175 gráficos
SKIP_PLOTTING = True 

# --- GALAXIAS A AJUSTAR ---
# Se procesarán todas las galaxias presentes en el archivo de datos.
GALAXIES_TO_FIT = 'ALL' # Indicador de que se usará la muestra completa.
...
def fit_galaxy_curve(df_rc, do_plotting):
    ...
    bounds = (
        ...
        [5.0, 5.0, 500000.0, 100.0, 5.0]  # Límite superior: Ampliado para galaxias masivas
    )
    ...
    try:
        ...
        # 6. Preparar resultados (Mapeo de V_max_sq -> A y R_scale -> R0)
        fit_results = {
            'Galaxy': galaxy_name, 
            'Yd': popt[0], 'Yb': popt[1], 
            'A': popt[2], 'R0': popt[3], 
        ...
        # 7. Graficar (solo si do_plotting es True)
        if do_plotting:
...
def load_sparc_data(filepath, galaxies_to_fit_list):
    ...
    try:
        ...
        # Filtrar solo si no estamos usando la lista completa ('ALL')
...
if __name__ == '__main__':
    ...
    # Cargar todos los nombres de galaxias únicas si GALAXIES_TO_FIT es 'ALL'
    if GALAXIES_TO_FIT == 'ALL':
        temp_df = load_sparc_data(DATA_FILE, 'temp') # Cargar temporalmente para obtener IDs
        if temp_df is None:
            exit()
        ...
        print(f"--- AJUSTE DE CURVAS DE ROTACIÓN EDR: MUESTRA COMPLETA (N={len(galaxy_list)}) ---")
    else:
        galaxy_list = GALAXIES_TO_FIT
        df_sparc = load_sparc_data(DATA_FILE, galaxy_list)
        print("--- AJUSTE DE CURVAS DE ROTACIÓN EDR: MUESTRA DE VALIDACIÓN (N=10) ---")
    ...
    for i, galaxy in enumerate(galaxy_list):
        ...
        if (i + 1) % 20 == 0 or i == 0 or i == len(galaxy_list) - 1:
            print(f"\nProcesando galaxia {i+1}/{len(galaxy_list)}: {galaxy}...")
    ...
    print("\n¡Ahora puedes ejecutar 'run_mdar_validation.py' para la validación MDAR con los nuevos datos!")
